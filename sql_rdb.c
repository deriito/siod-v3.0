...
LISP cadr(LISP x) { return (car(cdr(x))); }

LISP list3(LISP a, LISP b, LISP c) { return (cons(a, cons(b, cons(c, NIL)))); }
...
LISP sqlrtl_associations(void) { return (associations); }
...
LISP sqlrtl_statement_params(LISP x) { return ((get_statement(x))->params); }

LISP sqlrtl_statement_selects(LISP x) { return ((get_statement(x))->select_alist); }

LISP sqlrtl_statement_association(LISP x) { return ((get_statement(x))->association); }
